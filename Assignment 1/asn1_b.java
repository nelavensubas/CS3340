/*
 * Assignment 1: Question 7b
 */

public class asn1_b implements Comparable<asn1_b> {
	...
	public void times(asn1_b x) {
		...
		for (int i = 0; i < x.length(); i++) { // for each digit in x, call the multiplyByInt helper
			...
			this.plus(new asn1_b(multiplyByInt(digit, num, i)));
		}
	}
	 ...
	 */
	public static asn1_b fibonacciOnTime(int n) {
		if (n <= 1) {
			return new asn1_b(n);
		} else if (n == 0) {
			return new asn1_b(0);
	 ...
	 * @param A the matrix
	 * 
	 * @param N raise to the power N and put the result in A[][]
	 */
	...
	public static void multiplyMatrix(asn1_b[][] A, asn1_b[][] B) {
		asn1_b x1 = new asn1_b(A[0][0].getValue());
		x1.times(B[0][0]);
		asn1_b x2 = new asn1_b(A[0][1].getValue());
		x2.times(B[1][0]);
		x1.plus(x2);

		asn1_b y1 = new asn1_b(A[0][0].getValue());
		y1.times(B[0][1]);
		asn1_b y2 = new asn1_b(A[0][1].getValue());
		y2.times(B[1][1]);
		y1.plus(y2);

		asn1_b z1 = new asn1_b(A[1][0].getValue());
		z1.times(B[0][0]);
		asn1_b z2 = new asn1_b(A[1][1].getValue());
		z2.times(B[1][0]);
		z1.plus(z2);

		asn1_b w1 = new asn1_b(A[1][0].getValue());
		w1.times(B[0][1]);
		asn1_b w2 = new asn1_b(A[1][1].getValue());
		w2.times(B[1][1]);
		w1.plus(w2);
